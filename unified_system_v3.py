...
class UnifiedCommunityManagerSystem:
    ...
    async def launch_viral_video_campaign(
        ...
        try:
            ...
            logger.info(
                f"✅ Engagement: {engagement_strategy.get('actions_scheduled')} acciones programadas"
            )
    ...
    async def monitor_and_viralize_channel(
        ...
        try:
            ...
            if campaigns_today >= max_campaigns_per_day:
                logger.warning(
                    f"⚠️ Daily campaign limit reached: {campaigns_today}/{max_campaigns_per_day}"
                )
            ...
            for video in new_videos:
                ...
                logger.info(
                    f"      Score: {virality_score:.2f} {'✅' if virality_score >= virality_threshold else '❌'}"
                )
            ...
            logger.info(
                f"🚀 Lanzando {len(videos_to_launch)} campañas (quedan {remaining_slots} slots)"
            )
            ...
            for i, video in enumerate(videos_to_launch, 1):
                ...
                if auto_launch:
                    ...
                    logger.info(
                        f"📧 NOTIFICACIÓN: Video '{video['title']}' tiene score {video['virality_score']:.2f}"
                    )
            ...
            logger.info(
                f"   Campañas hoy: {monitor_state['campaigns_launched_today']}/{max_campaigns_per_day}"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="🚀 Unified System V3 - Community Manager")
    ...
    parser.add_argument(
        "--virality-threshold", type=float, default=0.70, help="Score mínimo (0.0-1.0)"
    )
    ...
    # ============================================
        ...
        print(
            f"📊 Campañas hoy: {monitor_result.get('campaigns_launched_today')}/{args.max_campaigns_per_day}"
        )
