...
from social_extensions.meta.advanced_campaign_system.dynamic_geo_adjustments import (
    DynamicGeoAdjuster,
)
from social_extensions.meta.advanced_campaign_system.follower_exclusion import (
    FollowerExclusionManager,
)
...
def test_modulos_refinados_completo():
    ...
    content_data = {
        ...
        "genre": "reggaeton",
        "collaboration": {"artist_id": "anuel_aa", "type": "featuring"},
    ...
    print(
        f"   🤝 Colaboración: {granular_tags.collaboration_artist} ({granular_tags.collaboration_type})"
    )
    ...
    campaign_with_exclusion = exclusion_manager.apply_exclusion_to_campaign(
        campaign_data, account_id="acc_reggaeton_artist_main"
    )
    ...
    avg_exclusion = sum(seg.exclusion_percentage for seg in filtered_segments) / len(
        filtered_segments
    )
    ...
    geo_adjustment_results = geo_adjuster.run_dynamic_adjustment_cycle(
        campaign_geo_data, hours_elapsed=18
    )
    ...
    if geo_adjustment_results["adjustments_executed"]:
        ...
        print(
            f"   ✅ No se requirieron ajustes: {geo_adjustment_results.get('reason', 'performance óptima')}"
        )
    print()
    ...
    final_geo_allocation = geo_adjustment_results.get(
        "updated_allocation", campaign_geo_data["geo_allocation"]["region_budgets"]
    )
    ...
    for i in range(1, 6):  # 5 clips
        ...
        base_views = int(
            (sum(final_geo_allocation.values()) / 5) / base_cpv
        )  # Budget per clip / cpv

        clip_performance_data[clip_id] = {
            "roi": ((base_views * base_ctr / 100 * 15) - (sum(final_geo_allocation.values()) / 5))
            / (sum(final_geo_allocation.values()) / 5)
            * 100,
            "ctr": base_ctr,
            "cpv": base_cpv,
            "views": base_views,
    ...
    campaign_initial_results = {
        ...
        "total_roi": sum(metrics["roi"] for metrics in clip_performance_data.values())
        / len(clip_performance_data),
    ...
    followup_results = followup_automator.run_automated_follow_up_sequence(
        campaign_initial_results, additional_budget=50
    )
    ...
    if followup_results["automation_executed"]:
        ...
        print(
            f"   🎪 ROI combinado: {combined_metrics['original_roi']:.1f}% → {combined_metrics['combined_roi']:.1f}%"
        )
        ...
        print(
            f"   🏆 Mejor ciclo: {best_cycle['clip_id']} en {best_cycle['platform']} ({best_cycle['roi']:+.1f}% ROI)"
        )
    else:
    ...
    print(
        f"   💰 Inversión total: ${final_investment:.0f} (inicial: ${total_investment:.0f} + seguimiento: ${final_investment - total_investment:.0f})"
    )
    ...
    print(
        f"   3️⃣ Ajustes Geográficos: ${geo_adjustment_impact:.0f} mejora esperada por optimización"
    )
    ...
    for validation, status in validations.items():
        ...
        print(
            f"   {status_icon} {validation.replace('_', ' ').title()}: {'OK' if status else 'FALLO'}"
        )

    all_valid = all(validations.values())
    print()
    print(
        f"🎪 ESTADO FINAL: {'✅ MÓDULOS REFINADOS 100% OPERATIVOS' if all_valid else '⚠️ ALGUNOS MÓDULOS REQUIEREN AJUSTES'}"
    )
    ...
    return {
        ...
        "geo_adjustments": {
            ...
            "spain_final_percentage": (
                final_geo_allocation["ES"] / sum(final_geo_allocation.values())
            )
            * 100,
        },
        "followup_cycles": {
