...
from social_extensions.meta.advanced_campaign_system.ultralytics_integration import (
...
def test_integracion_completa():
    ...
    print(
        f"   🔥 Collaborator influence: {campaign_tags.ml_features['collaborator_influence']:.3f}"
    )
    ...
    print(
        f"✅ Distribución geográfica: ${total_geo_budget:.0f} en {len(geo_allocation.region_budgets)} países"
    )
    ...
    print(
        f"   📺 YouTube boost: ${ultralytics_results['scaling_execution']['youtube_boost_budget']:.0f}"
    )
    print()
    ...
    print(
        f"   📊 ROI proyectado: {budget_optimization['performance_projection']['estimated_roi']:.1f}%"
    )
    print()
    ...
    total_investment_final = (
        total_optimized_budget + ultralytics_results["scaling_execution"]["youtube_boost_budget"]
    )

    # Calcular ROI final combinado
    estimated_revenue = total_investment_final * (
        budget_optimization["performance_projection"]["estimated_roi"] / 100
    )
    ...
    print(
        f"   • Vistas campaña principal: {sum(metrics['views'] for metrics in clip_performance_data.values()):,}"
    )
    print(
        f"   • Vistas YouTube boost: {ultralytics_results['scaling_execution']['youtube_boost_results']['boost_views']:,}"
    )
    print()
    ...
    print()
    print(
        f"🎯 ESTADO FINAL: {'✅ SISTEMA COMPLETAMENTE OPERATIVO' if all_valid else '❌ ERRORES DETECTADOS'}"
    )
