...
with tab1:
    ...
    if st.session_state.campaign_launched and st.session_state.campaign_results:
        ...
        with col2:
            total_accounts = len(
                results.get("platforms", {}).get("tiktok", {}).get("accounts", [])
        ...
        if "youtube" in platforms:
            st.markdown(
                f"🎥 **YouTube:** [{platforms['youtube']['url']}]({platforms['youtube']['url']})"
            )
        ...
        if "twitter" in platforms:
            st.markdown(
                f"🐦 **Twitter:** [{platforms['twitter']['url']}]({platforms['twitter']['url']})"
            )
        ...
        if st.button("🎯 Optimizar Campaña", type="primary"):
            with st.spinner("Analizando performance y aplicando optimizaciones..."):
                ...
                if optimizations.get("optimizations_applied", 0) > 0:
                    st.success(
                        f"✅ {optimizations['optimizations_applied']} optimizaciones aplicadas"
                    )

                    st.markdown(
                        f"**Impacto Estimado:** {optimizations.get('estimated_impact', 'N/A')}"
                    )
                    ...
                    for opt in optimizations.get("optimizations", []):
                        with st.expander(
                            f"🔧 {opt['action'].replace('_', ' ').title()} - {opt['platform']}"
                        ):
                            st.json(opt)
                else:
