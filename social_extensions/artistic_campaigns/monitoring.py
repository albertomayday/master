...
class ArtisticCampaignMonitor:
    ...
    async def _check_artistic_alert_conditions(self, campaign_id: str):
        ...
        appreciation_metrics = self.real_time_metrics.get(
            f"{metrics_key_prefix}artistic_appreciation"
        )
            ...
            if len(recent_values) >= 2:
                ...
                if current < previous * (1 - self.alert_thresholds["artistic_appreciation_drop"]):
                    await self._create_artistic_alert(
                        ...
                        MetricType.ARTISTIC_APPRECIATION,
    ...
    async def _track_learning_progress(self, campaign_id: str):
        ...
        recent_patterns = [
            ...
            and "generated_at" in p
            and datetime.fromisoformat(p["generated_at"]) > datetime.now() - timedelta(hours=1)
        ]
        ...
        if optimization_history:
            ...
            recent_optimizations = [
                ...
                if "applied_at" in opt
                and datetime.fromisoformat(opt["applied_at"]) > datetime.now() - timedelta(hours=6)
            ]
    ...
    async def _calculate_artistic_health(self, campaign_id: str) -> ArtisticHealthScore:
        ...
        if learning_metrics:
            recent_learning = [
                m for m in learning_metrics if m.timestamp > datetime.now() - timedelta(hours=24)
            ]
            learning_progress = (
                statistics.mean([m.value for m in recent_learning]) * 100 if recent_learning else 0
            )
        else:
            learning_progress = 0
    ...
    async def _generate_artistic_insights(self, campaign_id: str):
        ...
        if len(performance_history) >= 5:
            ...
            if len(appreciation_trend) >= 3:
                trend_slope = (appreciation_trend[-1] - appreciation_trend[0]) / len(
                    appreciation_trend
                )

                if trend_slope > 0.02:  # Improving
                    insights.append(
                        "Artistic appreciation is trending upward - current approach is resonating"
                    )
                elif trend_slope < -0.02:  # Declining
                    insights.append(
                        "Artistic appreciation declining - consider creative strategy review"
                    )
        ...
        if resonance_metrics and len(resonance_metrics) >= 10:
            ...
            resonance_volatility = (
                statistics.stdev(recent_resonance) if len(recent_resonance) > 1 else 0
            )

            if resonance_volatility > 0.2:
                insights.append(
                    "High creative resonance volatility - audience response is inconsistent"
                )
        ...
        for campaign_id in all_campaigns:
            ...
            if performance_history:
                avg_engagement = statistics.mean(
                    [p.engagement_rate for p in performance_history[-5:]]
                )
            ...
            if len(performances) >= 2:
                ...
                if avg_performance > 0.08:  # High performance threshold
                    cross_insights.append(
                        f"'{medium}' medium showing strong performance (avg: {avg_performance:.1%})"
                    )
    ...
    async def _send_artistic_alert_notification(self, alert: ArtisticAlert):
        ...
        self.logger.info(
            f"📢 Artistic alert notification: {json.dumps(notification_data, indent=2)}"
        )
    ...
    async def _cleanup_old_monitoring_data(self):
        ...
        for campaign_id in list(self.learning_metrics.keys()):
            self.learning_metrics[campaign_id] = [
                m for m in self.learning_metrics[campaign_id] if m.timestamp >= cutoff_time
            ]
