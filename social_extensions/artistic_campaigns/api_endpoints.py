...
class AudienceSegmentAPI(BaseModel):
    ...
    preferred_platforms: List[str] = Field(
        default=["instagram", "twitter"], description="Preferred platforms"
    )
...
async def create_artistic_campaign(
    ...
    try:
        ...
        if request.enable_continuous_learning:
            background_tasks.add_task(system.continuous_learning_cycle, result["campaign_id"])
...
async def get_campaign_performance(
    campaign_id: str, hours: int = 24, system: ArtisticCampaignSystem = Depends(get_artistic_system)
    ...
    try:
        ...
        recent_metrics = [m for m in performance_history if m.timestamp >= cutoff_time]
...
async def analyze_artistic_content(
    content: ArtisticContentAPI, system: ArtisticCampaignSystem = Depends(get_artistic_system)
):
...
async def get_system_status(system: ArtisticCampaignSystem = Depends(get_artistic_system)):
    ...
    try:
        ...
        for campaign_id, campaign_data in system.active_campaigns.items():
            ...
            campaign_summary = {
                ...
                "latest_engagement": (
                    latest_performance.engagement_rate if latest_performance else 0
                ),
            }
