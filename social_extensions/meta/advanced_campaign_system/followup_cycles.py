...
class FollowUpCyclesAutomator:
    ...
    def execute_follow_up_cycle(
        ...
        print(
            f"🔥 Engagement: {improved_engagement:.1f}% (+{((improved_engagement/base_engagement-1)*100):+.1f}%)"
        )
    ...
    def run_automated_follow_up_sequence(
        ...
        print(
            f"   📈 ROI combinado: {original_roi:.1f}% → {combined_roi:.1f}% ({combined_roi - original_roi:+.1f}%)"
        )
