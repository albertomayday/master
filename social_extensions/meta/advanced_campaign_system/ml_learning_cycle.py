...
class MLLearningCycle:
    ...
    def save_campaign_cycle_data(self, campaign_results: Dict) -> str:
        ...
        print(
            f"📊 Métricas: {len(cycle_metrics.clip_performance)} clips, {len(cycle_metrics.geo_performance)} países"
        )
        print(
            f"💰 Inversión: ${cycle_metrics.total_investment} | ROI: {cycle_metrics.total_roi:.1f}%"
        )
        print()
    ...
    def predict_next_campaign_performance(
        ...
        print(
            f"📊 ROI predicho: {adjusted_predictions['roi']:.1f}% (confianza: {model_adjustments.confidence_score:.2f})"
        )
    ...
    def generate_recommendations(
        ...
        if predictions["roi"] > 200:
            recommendations.append(
                "🚀 ROI excelente predicho - Considerar aumentar presupuesto 20%"
            )
