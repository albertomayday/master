...
class BudgetOptimizer:
    ...
    def simulate_initial_cycle(
        self, clips_data: List[ClipData]
    ) -> Tuple[Dict[str, ClipMetrics], str]:
        """
    ...
    def execute_reinvestment_cycle(self, winner_clip: str, winner_metrics: ClipMetrics) -> Dict:
        ...
        additional_impressions = (
            self.youtube_reinvestment / (winner_metrics.cpc * youtube_cpm_factor)
        ) * 1000
        ...
        youtube_metrics = {
            ...
            "youtube_roi": self.calculate_youtube_roi(
                additional_views + int(organic_boost), self.youtube_reinvestment
            ),
        }
    ...
    def calculate_viral_coefficient(self, metrics: ClipMetrics) -> float:
        ...
        viral_coefficient = (
            (engagement_factor * 0.5) + (ctr_factor * 0.3) + (conversion_factor * 0.2)
        )
    ...
    def simulate_complete_budget_cycle(self, clips_data: List[ClipData]) -> Dict:
        ...
        total_views = (
            sum([metrics.views for metrics in initial_results.values()])
            + youtube_results["total_youtube_views"]
        )
...
if __name__ == "__main__":
    ...
    clips_example = [
        ...
        ClipData(
            "clip_002", "trap", "trap_melodico", ["ES", "CO"], 25, "audience_colaborador", 0.85
        ),
