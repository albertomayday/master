...
class PerformancePredictor:
    ...
    def __init__(self):
        ...
        self.genre_multipliers = {"trap": 1.15, "reggaeton": 1.25, "rap": 1.05, "corrido": 0.95}
    ...
    def load_historical_data(
        self, data_path: str = "/workspaces/master/data/clip_performance_history.json"
    ):
            ...
            performance = ClipPerformanceData(
                ...
                demographic_breakdown={
                    "18-24": {
                        ...
                        "engagement": random.uniform(3, 7),
                    },
                    "25-34": {
                        "views": random.randint(150, 600),
                        "engagement": random.uniform(2, 5),
                    },
                },
    ...
    def predict_clip_performance(
        self, clip_analysis: ClipAnalysis, budget: float = 100
    ) -> Dict[str, float]:
        ...
        dominant_genre = max(
            clip_analysis.genre_classification, key=clip_analysis.genre_classification.get
        )
        ...
        predicted_revenue = (
            predicted_views * predicted_ctr / 100 * 15
        )  # $15 por conversión simulada
...
class UltralyticsClipSelector:
    ...
    def analyze_clip_batch(self, clip_paths: List[str]) -> List[ClipAnalysis]:
        ...
        for i, clip_path in enumerate(clip_paths, 1):
            ...
            print(
                f"   🎵 Género dominante: {max(analysis.genre_classification, key=analysis.genre_classification.get)}"
            )
    ...
    def select_best_clips(
        ...
        for analysis in clip_analyses:
            ...
            score = (
                ...
                + (performance_pred["predicted_roi"] / 200)
                * self.selection_weights["predicted_roi"]
    ...
    def create_scaling_decisions(
        self, selected_clips: List[Tuple[ClipAnalysis, Dict]], total_budget: float = 500
...
class AutomaticClipScaler:
    ...
    def execute_scaling_plan(self, scaling_decisions: List[ScalingDecision]) -> Dict:
        ...
        for decision in scaling_decisions:
            ...
            predicted_views = (
                decision.predicted_performance["predicted_views"] * decision.scaling_factor
            )
            predicted_revenue = (
                decision.predicted_performance["predicted_revenue"] * decision.scaling_factor
            )
            ...
            scaled_clips.append(
                {
                    ...
                    "predicted_revenue": predicted_revenue,
                    "youtube_boost": (
                        youtube_budget if predicted_roi > self.reinvestment_threshold else 0
                    ),
        ...
        # Calcular ROI total
        total_roi = (
            ((total_predicted_revenue - total_investment) / total_investment) * 100
...
class UltralyticsIntegration:
    ...
    def execute_complete_flow(
        self, clip_directory: str = "/workspaces/master/data/video_clips/"
    ) -> Dict:
        """
        ...
        selected_clips = self.clip_selector.select_best_clips(
            clip_analyses, self.default_clip_budget, self.max_clips_to_select
        )

        # 4. Crear decisiones de escalado
        scaling_decisions = self.clip_selector.create_scaling_decisions(
            selected_clips, self.total_scaling_budget
        )
            ...
            "clip_analyses": [
                {
                    ...
                    "dominant_genre": max(
                        analysis.genre_classification, key=analysis.genre_classification.get
                    ),
...
if __name__ == "__main__":
    ...
    print(
        f"   🎥 Vistas YouTube: {results['scaling_execution']['youtube_boost_results']['boost_views']:,}"
    )
