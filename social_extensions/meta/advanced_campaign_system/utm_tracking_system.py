...
class UTMMLIntegration:
    ...
    def generate_ml_insights(self) -> Dict[str, Any]:
        ...
        insights.append(
            f"Mejor subgénero por conversión: {best_subgenre[0]} ({best_subgenre[1]['conversion_rate']:.1f}%)"
        )
        ...
        for subgenre, perf in subgenre_performance.items():
            if perf["conversion_rate"] > 5.0:  # Alta conversión
                recommendations.append(
                    f"Incrementar presupuesto en {subgenre} (alta conversión: {perf['conversion_rate']:.1f}%)"
                )
            elif perf["conversion_rate"] < 1.0:  # Baja conversión
                recommendations.append(
                    f"Optimizar o pausar {subgenre} (baja conversión: {perf['conversion_rate']:.1f}%)"
                )
