...
async def demo_sistema_musical_completo():
    ...
    trap_pixel = create_trap_pixel(
        pixel_id="TrapPixel01", name="Urban Trap España 2024", theme="street_life"
    )
    ...
    reggaeton_pixel = create_reggaeton_pixel(
        pixel_id="ReggaetonPixel01", name="Latin Party Hits", theme="party_vibes"
    )
    ...
    corrido_pixel = create_corrido_pixel(
        pixel_id="CorridoPixel01", name="Corridos Tumbados México", theme="narco_story"
    )
    ...
    print(
        f"   💃 Campaña Reggaetón Colombia: €{reggaeton_campaign.budget} | {reggaeton_campaign.target_age_range}"
    )
    ...
    print(
        f"   🤠 Campaña Corrido México: €{corrido_campaign.budget} | {corrido_campaign.target_age_range}"
    )
    ...
    for campaign in campaigns:
        ...
        print(
            f"      CTR Predicho: {prediction.predicted_ctr:.3f} ({prediction.predicted_ctr/0.02:.1f}x baseline)"
        )
        ...
        print(
            f"      🎯 Insights: {prediction.genre_specific_insights[0] if prediction.genre_specific_insights else 'N/A'}"
        )
    ...
    for campaign in campaigns:
        ...
        print(
            f"      Conversiones: {campaign.conversions} | Watch Time: {campaign.current_watch_time:.1f}s"
        )
    ...
    for campaign in campaigns:
        ...
        print(
            f"      🚀 Acción Recomendada: {insights.optimization_decision.decision_type.upper()}"
        )
        ...
        if insights.viral_potential:
            print(
                f"      🔥 ¡POTENCIAL VIRAL DETECTADO! Confianza: {insights.viral_potential['confidence']:.1%}"
            )

        if insights.scaling_opportunity:
            print(
                f"      📈 Oportunidad de Escalar: {insights.scaling_opportunity['recommended_multiplier']:.1f}x"
            )
    ...
    sorted_genres = sorted(genre_performance.items(), key=lambda x: x[1]["avg_ctr"], reverse=True)
...
async def test_sistema_basico():
    ...
    print(
        f"✅ Predicción ML: CTR {prediction.predicted_ctr:.3f}, confianza {prediction.confidence:.1%}"
    )
