...
class MetaAdsAutomator:
    ...
    async def _create_adset(self, campaign_id: str, brief: CampaignBrief, adset_name: str) -> str:
        ...
        daily_budget = (brief.budget_total / len(brief.creatives)) / (
            (brief.end_date - brief.start_date).days or 1
        )
    ...
    async def _create_creative(self, creative: Creative, brief: CampaignBrief) -> str:
        ...
        if creative.type == CreativeType.SINGLE_IMAGE and creative.image_url:
            ...
            creative_params[AdCreative.Field.object_story_spec]["link_data"]["image_hash"] = image[
                AdImage.Field.hash
            ]
        ...
        try:
            ...
            for ad_id in campaign_info["ad_ids"]:
                ...
                insights = ad.get_insights(
                    ...
                    params={
                        "time_range": {
                            "since": (datetime.now() - timedelta(days=date_range)).strftime(
                                "%Y-%m-%d"
                            ),
                ...
                for insight in insights:
                    ...
                    conversion_value = float(
                        insight.get("conversion_values", [{"value": "0"}])[0]["value"]
                    )
    ...
    async def optimize_campaign(
        self, campaign_id: str, insights: List[MLInsight]
    ) -> Dict[str, Any]:
            ...
            for insight in insights:
                ...
                if action_type == "scale_budget":
                    result = await self._scale_adset_budget(
                        action["adset_id"], action["scale_factor"]
                    )
                    ...
                    result = await self._adjust_adset_targeting(
                        action["adset_id"], action["targeting_changes"]
                    )
    ...
    async def _scale_adset_budget(self, adset_id: str, scale_factor: float) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Scaled adset {adset_id} budget: {current_budget/100} -> {new_budget/100}"
            )
            return True
    ...
    async def _adjust_adset_targeting(
        self, adset_id: str, targeting_changes: Dict[str, Any]
    ) -> bool:
    ...
    async def _dummy_optimize_campaign(
        self, campaign_id: str, insights: List[MLInsight]
    ) -> Dict[str, Any]:
        """Dummy optimization for testing"""
        self.logger.info(
            f"🎭 DUMMY: Optimizing campaign {campaign_id} with {len(insights)} insights"
        )
