    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            background_tasks.add_task(
                schedule_optimization_check, result["campaign_id"], delay_hours=2
            )
...
async def get_campaign_metrics(
    campaign_id: str, hours: int = 24, automator: MetaAdsAutomator = Depends(get_meta_automator)
) -> MetricsResponse:
    ...
    try:
        ...
        if not metrics:
            raise HTTPException(
                status_code=404, detail=f"No metrics found for campaign {campaign_id}"
            )
...
async def optimize_campaign(
    ...
    try:
        ...
        if not metrics:
            raise HTTPException(
                status_code=404, detail=f"Campaign {campaign_id} not found or has no metrics"
            )
                ...
                if hasattr(action, "parameters"):
                    insights.append(
                        MLInsight(
                            ...
                            recommended_action={"type": action.action_type, **action.parameters},
        ...
        background_tasks.add_task(schedule_optimization_check, campaign_id, delay_hours=6)
...
async def get_account_health(
    account_id: str, automator: MetaAdsAutomator = Depends(get_meta_automator)
...
async def handle_conversion_webhook(
    data: Dict[str, Any], automator: MetaAdsAutomator = Depends(get_meta_automator)
):
...
def initialize_meta_endpoints(meta_config: Dict[str, Any], action_config: Dict[str, Any]):
