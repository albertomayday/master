...
from .musical_ml_models import MLPrediction, MusicalMLEnsemble, musical_ml_ensemble
...
class MusicalIntelligenceEngine:
    ...
    async def _generate_optimization_decision(
        self, campaign: CampaignContext, ml_prediction: MLPrediction
        ...
        if ml_prediction.prob_scale_worthy > self.scale_up_threshold:
            ...
            reasoning.append(
                f"Alta probabilidad de éxito al escalar ({ml_prediction.prob_scale_worthy:.2%})"
            )
            ...
            expected_improvement = {"ctr_improvement": 0.25, "cpv_improvement": -0.15}
            ...
            reasoning.append(
                f"¡POTENCIAL VIRAL DETECTADO! ({ml_prediction.prob_viral_potential:.2%})"
            )
            reasoning.append("Escalar agresivamente para capitalizar momento viral")
            expected_improvement = {"viral_reach_multiplier": 5.0, "brand_awareness_boost": 3.0}
    ...
    async def _calculate_genre_percentile(self, campaign: CampaignContext) -> float:
        ...
        genre_campaigns = [
            c
            for c in self.context_db.campaigns.values()
    ...
    def _determine_market_position(
        self, campaign: CampaignContext, prediction: MLPrediction
    ) -> str:
    ...
    async def execute_optimization_decision(self, decision: OptimizationDecision) -> Dict[str, Any]:
        ...
        logger.info(
            f"Executing optimization: {decision.decision_type} for campaign {decision.campaign_id}"
        )
    ...
    def get_genre_intelligence_summary(self, genre: MusicalGenre) -> Dict[str, Any]:
        ...
        avg_cpv = sum(c.current_cpv for c in genre_campaigns if c.current_cpv > 0) / max(
            1, len([c for c in genre_campaigns if c.current_cpv > 0])
        )
        ...
        return {
            ...
            "optimization_opportunities": len(
                [
                    d
                    for d in self.optimization_history
