...
class MetaAdsMonitor:
    ...
    def _find_similar_alert(
        self, campaign_id: str, metric_type: MetricType, title: str
    ) -> Optional[MetaAlert]:
    ...
    async def _calculate_campaign_health(
        self, campaign_id: str, metrics: List
    ) -> CampaignHealthScore:
    ...
    async def _calculate_technical_score(self, campaign_id: str) -> float:
        ...
        critical_count = sum(
            1 for alert in campaign_alerts if alert.severity == AlertSeverity.CRITICAL
        )
        error_count = sum(1 for alert in campaign_alerts if alert.severity == AlertSeverity.ERROR)
        warning_count = sum(
            1 for alert in campaign_alerts if alert.severity == AlertSeverity.WARNING
        )
    ...
    async def _send_daily_report(self):
        ...
        report_data = {
            ...
            "health_scores": {
                cid: score.overall_score for cid, score in self.campaign_health_scores.items()
            },
        }
    ...
    async def _send_weekly_report(self):
        ...
        for campaign_id, history in self.performance_history.items():
            if len(history) >= 7:
                ...
                weekly_metrics[campaign_id] = {"avg_roas": avg_roas, "avg_cpa": avg_cpa}
        ...
        report_data = {
            ...
            "alert_summary": {
                severity.value: count for severity, count in self.alert_counts.items()
            },
        }
...
def get_monitoring_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "notifications": {"slack_webhook_url": None, "email_recipients": [], "webhook_urls": []},
        "reporting": {
