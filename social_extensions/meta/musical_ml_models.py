...
class TrapModel(BaseGenreModel):
    ...
    def predict(self, features: MLFeatures) -> MLPrediction:
        ...
        prob_scale_worthy = (
            prob_high_performance * 0.8 if predicted_ctr > self.viral_threshold else 0.3
        )
        prob_viral_potential = (
            (predicted_ctr / self.viral_threshold) * 0.4
            if predicted_ctr > self.viral_threshold * 0.8
            else 0.1
        )
        ...
        if features.audience_match_score < 0.6:
            insights.append(
                "Audiencia no optimizada para trap - considerar targeting masculino 18-25"
            )
        if features.budget < 300:
            insights.append(
                "Presupuesto bajo para trap - incrementar a 400-600€ para mejor performance"
            )
        ...
        return MLPrediction(
            ...
            confidence=0.75
            + features.pixel_total_campaigns * 0.01,  # More campaigns = more confidence
    ...
    def train(self, training_data: List[CampaignContext]) -> Dict[str, Any]:
        ...
        trap_campaigns = [
            c for c in training_data if c.pixel_profile.musical_context.genre == MusicalGenre.TRAP
        ]

        if len(trap_campaigns) < 10:
            return {
                "error": "Insufficient trap training data",
                "campaigns_needed": 10 - len(trap_campaigns),
            }
...
class ReggatonModel(BaseGenreModel):
    ...
    def predict(self, features: MLFeatures) -> MLPrediction:
        ...
        prob_viral_potential = (
            (predicted_ctr / self.viral_threshold) * 0.6
        ...
        if features.audience_match_score > 0.8:
            insights.append(
                "Excelente match de audiencia para reggaetón - alta probabilidad de viralización"
            )
    ...
    def train(self, training_data: List[CampaignContext]) -> Dict[str, Any]:
        ...
        reggaeton_campaigns = [
            ...
            if c.pixel_profile.musical_context.genre == MusicalGenre.REGGAETON
        ]
        ...
        return {
            ...
            "viral_rate": (
                len(viral_campaigns) / len(reggaeton_campaigns) if reggaeton_campaigns else 0
            ),
        }
...
class MusicalMLEnsemble:
    ...
    def predict_campaign_performance(self, campaign: CampaignContext) -> MLPrediction:
        ...
        prediction.genre_specific_insights.insert(
            0, f"Predicción usando modelo especializado en {genre.value}"
        )
    ...
    def _generic_prediction(self, campaign: CampaignContext) -> MLPrediction:
        ...
        return MLPrediction(
            ...
            genre_specific_insights=[
                "Usando modelo genérico - considera crear modelo especializado"
            ],
