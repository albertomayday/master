...
try:
    from telegram_automation.database.models import (
        Contact,
        ContactStatus,
        ConversationContext,
        ConversationState,
...
class TelegramBot:
    ...
    def __init__(
        self, api_id: int, api_hash: str, bot_token: str, database_url: str = "sqlite:///dummy.db"
    ):
        self.api_id = api_id
        self.api_hash = api_hash
...
class TelegramLike4LikeBot:
    ...
    async def process_like4like_message(self, event, sender, classification):
        ...
        try:
            logger.info(
                f"🎯 Detected like4like message from @{getattr(sender, 'username', 'unknown')} (confidence: {classification['confidence']:.2f})"
            )
            ...
            if existing_contact:
                ...
                if (
                    existing_contact.status == ContactStatus.ACTIVE_SAVED.value
                    and existing_contact.reliability_score >= 70
                ):
                    logger.info(
                        f"   ℹ️  Contact {existing_contact.username} is already active and reliable, skipping"
                    )
                    return

                # If we recently contacted them, skip
                if (
                    existing_contact.last_contact_at
                    and existing_contact.last_contact_at > datetime.now() - timedelta(days=3)
                ):
            ...
            if not existing_contact:
                contact = Contact(
                    ...
                    original_video_url=(
                        classification["youtube_urls"][0]
                        if classification["youtube_urls"]
                        else None
                    ),
    ...
    async def send_initial_dm(self, contact: Contact, classification: Dict[str, Any]):
        ...
        try:
            ...
            their_video_url = (
                classification["youtube_urls"][0] if classification["youtube_urls"] else None
            )
    ...
    async def craft_initial_message(
        self, contact: Contact, their_video_url: Optional[str], extracted_terms: Dict[str, Any]
    ) -> str:
    ...
    def generate_initial_terms(self, extracted_terms: Dict[str, Any]) -> Dict[str, int]:
        ...
        # Default terms
        terms = {"likes": 5, "subs": 1, "comments": 1, "watch_seconds": 60}
    ...
    async def handle_private_message(self, event):
        ...
        try:
            ...
            logger.info(
                f"💬 Received DM from @{getattr(sender, 'username', 'unknown')}: {message_text[:100]}..."
            )
            ...
            if contact.first_contact_at:
                response_time_minutes = int(
                    (datetime.now() - contact.first_contact_at).total_seconds() / 60
                )
    ...
    async def process_conversation_response(
        ...
        # For now, just log
        logger.info(
            f"🔄 Processing response in state {conversation_state.current_state}: {message_text[:50]}..."
        )
    ...
    async def log_message(
        ...
        await self.db.execute_command(
            ...
            message_id,
            message_text,
            classification["youtube_urls"],
            classification["is_like4like"],
            classification["confidence"],
            classification["extracted_terms"],
            datetime.now(),
        )
