...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/apply_system.log"), logging.StreamHandler()],
)
...
class ProjectApplySystem:
    ...
    async def analyze_python_code_quality(self):
        ...
        python_files = [
            ...
            if not any(
                exclude in str(f) for exclude in ["__pycache__", ".venv", "node_modules", ".git"]
            )
        ]
        ...
        for py_file in python_files:
            try:
                ...
                if '"""' in content or "'''" in content:
                    ...
                    metrics["files_without_docstrings"].append(
                ...
                if len(lines) > 500:
                    metrics["long_files"].append(
                        {"file": str(py_file.relative_to(self.project_root)), "lines": len(lines)}
                    )
        ...
        logger.info(
            f"  • Files with docstrings: {metrics['files_with_docstrings']}/{metrics['total_files']} ({metrics['files_with_docstrings']/metrics['total_files']*100:.1f}%)"
        )
        logger.info(
            f"  • Files with type hints: {metrics['files_with_type_hints']}/{metrics['total_files']} ({metrics['files_with_type_hints']/metrics['total_files']*100:.1f}%)"
        )
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if any(
                        ...
                        if not any(
                            safe in line_lower
                            for safe in ["dummy", "test", "placeholder", "example"]
                        ):
    ...
    async def analyze_dependencies(self):
        ...
        self.code_quality_metrics["dependencies"] = {
            ...
            "total_unique_deps": len(
                set().union(*[req_file.read_text().split("\n") for req_file in req_files])
            ),
        }
        ...
        try:
            ...
            if ("class " in content or "def " in content) and "__all__" not in content:
                ...
                for line in content.split("\n"):
                    ...
                    ) and not line.startswith(("def _", "async def _")):
...
'''
    ...
    async def test_universal_wake(self):
        ...
        try:
            result = subprocess.run(
                ["make", "wake"], cwd=self.project_root, capture_output=True, text=True, timeout=120
            )
            ...
            await asyncio.sleep(3)
            subprocess.run(
                ["python3", "awakener.py", "--stop"], cwd=self.project_root, capture_output=True
            )
    ...
    async def test_cross_branch_compatibility(self):
        ...
        logger.info(
            f"🔄 Cross-branch compatibility: {existing_files}/{len(env_files)} env files present"
        )
    ...
    async def test_error_recovery(self):
        ...
        logger.info(
            f"🚨 Error recovery test: {'✅ Passed' if validation_success else '⚠️ Issues detected'}"
        )
...
"""
        ...
        self.apply_results["documentation"] = {"readme_created": True, "files_documented": 1}
...
"""
    ...
    async def generate_final_report(self):
        ...
        report_path = (
            self.project_root
            / "logs"
            / f'apply_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
        ...
        if "python" in self.code_quality_metrics:
            ...
            if len(metrics["long_files"]) > 0:
                recommendations.append(
                    "Consider refactoring long files (>500 lines) into smaller modules"
                )
    ...
    def _print_final_summary(self, report: Dict[str, Any]):
        ...
        if "python" in self.code_quality_metrics:
            ...
            logger.info(
                f"  • Documentation coverage: {metrics['files_with_docstrings']}/{metrics['total_files']} ({metrics['files_with_docstrings']/metrics['total_files']*100:.1f}%)"
            )
        ...
        logger.info(f"\n🌍 INTEGRATION TESTING:")
        logger.info(
            f"  • Universal wake: {'✅ PASSED' if universal_wake.get('success') else '❌ FAILED'}"
        )
        ...
        logger.info(
            f"  • Error handling improved: {'✅' if 'error_handling' in self.apply_results else '⏸️'}"
        )
        logger.info(f"  • Logging configured: {'✅' if 'logging' in self.apply_results else '⏸️'}")
        logger.info(
            f"  • Documentation updated: {'✅' if 'documentation' in self.apply_results else '⏸️'}"
        )
