    ...
    async def disable_capability(self, capability: PlatformCapability) -> bool:
        ...
        try:
            if capability in self.config.enabled_capabilities:
                ...
                self.logger.info(
                    f"❌ Disabled capability {capability.value} for {self.platform_id}"
                )
                return True
            return False
        except Exception as e:
...
class InstagramController(PlatformController):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.logger.info(
                f"🔄 Initializing Instagram controller in {self.config.mode.value} mode"
            )
...
class LinkedInController(PlatformController):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.logger.info(
                f"🔄 Initializing LinkedIn controller in {self.config.mode.value} mode"
            )
...
class WhatsAppController(PlatformController):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.logger.info(
                f"🔄 Initializing WhatsApp controller in {self.config.mode.value} mode"
            )
...
class UniversalPlatformToggleSystem:
    ...
    async def set_global_mode(self, mode: PlatformMode):
        ...
        for platform_id, config in self.platform_configs.items():
            ...
            if old_mode != mode:
                self.logger.info(
                    f"🔄 Switching {platform_id} from {old_mode.value} to {mode.value}"
                )
    ...
    async def enable_platform_capability(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error enabling capability {capability.value} for {platform_id}: {e}"
            )
            return False
    ...
    async def disable_platform_capability(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error disabling capability {capability.value} for {platform_id}: {e}"
            )
            return False
    ...
    async def _load_platform_configurations(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                self.logger.info(
                    f"📖 Loaded configuration for {len(self.platform_configs)} platforms"
                )
    ...
    async def _create_default_configurations(self):
        ...
        self.logger.info(
            f"📝 Created default configurations for {len(self.platform_configs)} platforms"
        )
