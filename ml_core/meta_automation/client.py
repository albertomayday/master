...
class MetaMarketingClient:
    ...
    async def analyze_video(self, video_id: str) -> Dict[str, Any]:
        ...
        response = await client.post("/api/v1/meta/analyze-video", params={"video_id": video_id})
        response.raise_for_status()
        return response.json()

    async def generate_variations(
        self, video_id: str, segment_index: int, count: int = 5
    ) -> Dict[str, Any]:
        """Generar variaciones de anuncios para un segmento."""
        client = self._ensure_client()
        response = await client.post(
            "/api/v1/meta/generate-variations",
            params={"video_id": video_id, "segment_index": segment_index, "count": count},
        )
        response.raise_for_status()
        return response.json()

    async def create_campaign(
        self, video_id: str, pixel_id: str, genre: str, budget: float
    ) -> Dict[str, Any]:
        """Crear nueva campaña publicitaria."""
        client = self._ensure_client()
        response = await client.post(
            "/api/v1/meta/create-campaign",
            json={"video_id": video_id, "pixel_id": pixel_id, "genre": genre, "budget": budget},
        )
        response.raise_for_status()
        return response.json()

    async def optimize_campaign(
        self, campaign_id: str, date_range: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        ...
        response = await client.post(
            ...
            json={"campaign_id": campaign_id, "date_range": date_range},
        )
        response.raise_for_status()
        return response.json()
