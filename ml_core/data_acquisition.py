...
class RealTimeDataAcquisitionEngine:
    ...
    async def start_real_time_collection(self):
        ...
        self.logger.info(
            f"✅ Real-time collection started for {len(self.collection_tasks)} streams"
        )
    ...
    async def _platform_collection_loop(self, platform: str, config: DataCollectionConfig):
        ...
        while self.is_collecting:
            try:
                ...
                self.logger.debug(
                    f"📊 {platform} collection cycle: {len(metrics)} metrics in {loop_duration:.2f}s"
                )
    ...
    async def _archive_session_data(self):
        ...
        if self.real_time_metrics:
            ...
            self.logger.info(
                f"📁 Archived {len(self.real_time_metrics)} metrics to historical storage"
            )
