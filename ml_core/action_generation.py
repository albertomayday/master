...
try:
    from .cloud_processing import InsightType, MLInsight, MLPrediction
    from .data_acquisition import MetricType, SocialMetric
except ImportError:
    import sys

    sys.path.append("/workspaces/master")
    from ml_core.cloud_processing import InsightType, MLInsight, MLPrediction
    from ml_core.data_acquisition import MetricType, SocialMetric
...
class TwitterActionGenerator(ActionGenerator):
    ...
    async def _generate_twitter_engagement_actions(
        self, insight: MLInsight
...
class LinkedInActionGenerator(ActionGenerator):
    ...
    async def _generate_professional_engagement_actions(
        self, insight: MLInsight
    ) -> List[ExecutableAction]:
    ...
    async def _generate_thought_leadership_actions(
        self, insight: MLInsight
    ) -> List[ExecutableAction]:
...
class WhatsAppActionGenerator(ActionGenerator):
    ...
    async def _generate_customer_service_actions(
        self, insight: MLInsight
    ) -> List[ExecutableAction]:
...
class UniversalActionGenerationSystem:
    ...
    async def generate_actions_from_insight(self, insight: MLInsight) -> List[ExecutableAction]:
        ...
        self.logger.info(
            f"⚡ Generated {len(all_actions)} actions from insight {insight.insight_id}"
        )
        return all_actions
    ...
    async def execute_action_immediately(self, action: ExecutableAction) -> ActionResult:
        ...
        self.logger.info(
            f"🎯 Executing action immediately: {action.action_id} on {action.platform}"
        )
                ...
                platform: {
                    ...
                    "completed_today": len(
                        [
                            ...
                            and a.created_at.date() == datetime.now().date()
                        ]
                    ),
                }
            ...
            try:
                ...
                if ready_actions and len(self.executing_actions) < self.max_concurrent_actions:
                    ...
                    if execution_tasks:
                            ...
                            execution_tasks, return_when=asyncio.FIRST_COMPLETED, timeout=30
                        )
    ...
    async def _execute_single_action(self, action: ExecutableAction):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                self.logger.error(
                    f"❌ Attempt {attempt + 1} failed for action {action.action_id}: {e}"
                )
    ...
    async def _action_cleanup_loop(self):
        ...
        while self.is_executing:
            try:
                ...
                self.completed_actions = [
                    action for action in self.completed_actions if action.created_at >= cutoff_time
                ]

                self.failed_actions = [
                    action for action in self.failed_actions if action.created_at >= cutoff_time
                ]

                self.logger.info(
                    f"🧹 Cleaned up old actions - Completed: {len(self.completed_actions)}, Failed: {len(self.failed_actions)}"
                )
    ...
    def _apply_global_action_policies(self, action: ExecutableAction):
        ...
        platform_actions_today = len(
            [
                a
                for a in self.completed_actions + self.executing_actions + self.pending_actions
    ...
    async def _update_execution_metrics(self, platform: str, execution_time: float, success: bool):
        ...
        if success:
            ...
            platform_metrics["success_rate"] = (
                (current_success_rate * (total_executed - 1)) + 1
            ) / total_executed

            # Update execution time
            current_avg_time = platform_metrics["avg_execution_time"]
            platform_metrics["avg_execution_time"] = (
                (current_avg_time * (total_executed - 1)) + execution_time
            ) / total_executed
...
# Factory function
def create_action_generation_system(
