...
class YoloDemo:
    ...
    async def demo_single_device_analysis(self, device_serial: str = "demo_device"):
        ...
        try:
            ...
            print(
                f"     • Safe Actions: {len(mock_analysis['automation_recommendations']['safe_actions'])}"
            )
            print(
                f"     • Automation Confidence: {mock_analysis['automation_recommendations']['automation_confidence']:.2%}"
            )

            # Display detected elements
            for detection in mock_analysis["detections"]:
                print(
                    f"     • {detection['class_name']} (confidence: {detection['confidence']:.2%})"
                )
    ...
    async def demo_anomaly_detection(self, device_serial: str = "demo_device"):
        ...
        try:
            ...
            print(
                f"     • Recommendations: {len(high_anomaly['recommendations'])} actions suggested"
            )
        ...
        try:
            ...
            for engagement_type in engagement_types:
                ...
                if guidance["target_element"]:
                    ...
                    print(
                        f"     • Target: {element['class_name']} at ({element['coordinates']['center_x']}, {element['coordinates']['center_y']})"
                    )
    ...
    async def demo_task_queue_integration(self):
        ...
        try:
            ...
            for task_config in yolo_tasks:
                ...
                print(
                    f"     ✅ Created {task_config['task_type']} for {task_config['device_serial']}"
                )
