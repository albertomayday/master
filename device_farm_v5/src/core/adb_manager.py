...
from ..core.models import Device, get_db_session
...
class ADBDeviceManager:
    ...
    async def _run_adb_command(
        self, args: List[str], device_serial: Optional[str] = None
    ) -> Tuple[str, str]:
    ...
    async def _get_battery_level(self, serial: str) -> Optional[int]:
        ...
        try:
            stdout, _ = await self._run_adb_command(
                ["shell", "dumpsys", "battery", "|", "grep", "level"], serial
            )
            if stdout:
    ...
    async def _get_wifi_ip(self, serial: str) -> Optional[str]:
        ...
        try:
            stdout, _ = await self._run_adb_command(
                ["shell", "ip", "route", "|", "grep", "wlan0"], serial
            )
            if stdout:
    ...
    async def configure_proxy(self, serial: str, proxy_host: str, proxy_port: int) -> bool:
        ...
        try:
            ...
            proxy_settings = ProxySettings(host=proxy_host, port=proxy_port, enabled=True)
            ...
            stdout, stderr = await self._run_adb_command(
                ["shell", "settings", "put", "global", "http_proxy", proxy_string], serial
            )
    ...
    async def _verify_proxy(self, serial: str, proxy_host: str, proxy_port: int) -> bool:
        ...
        try:
            ...
            stdout, _ = await self._run_adb_command(
                ["shell", "settings", "get", "global", "http_proxy"], serial
            )
            ...
            if current == expected:
                ...
                test_url = (
                    self.config.raw_config.get("adb", {}).get("proxy", {}).get("test_url", "")
                )
                if test_url:
    ...
    async def _test_proxy_connectivity(self, serial: str, test_url: str) -> bool:
        ...
        try:
            ...
            stdout, stderr = await self._run_adb_command(
                ["shell", "curl", "-s", "--max-time", "10", test_url], serial
            )
    ...
    async def disable_proxy(self, serial: str) -> bool:
        ...
        try:
            ...
            # Clear proxy setting
            stdout, stderr = await self._run_adb_command(
                ["shell", "settings", "delete", "global", "http_proxy"], serial
            )
    ...
    async def _sync_devices_to_db(self, devices: List[DeviceInfo]):
        ...
        try:
            ...
            for device_info in devices:
                ...
                existing = session.query(Device).filter(Device.serial == device_info.serial).first()
