...
class AppiumServerConfig:
    ...
    def get_capabilities(self, app_package: Optional[str] = None) -> Dict[str, Any]:
        ...
        if app_package:
            caps.update(
                {"appPackage": app_package, "appActivity": self._get_main_activity(app_package)}
            )
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "--bootstrap-port",
                str(self.config.bootstrap_port),
                "--chromedriver-port",
                str(self.config.chromedriver_port),
                "--session-override",
                "--log-level",
                "info",
                "--log-timestamp",
            ...
            logger.info(
                f"Starting Appium server for device {self.config.device_serial} on port {self.config.port}"
            )
        ...
        try:
            ...
            if self.process:
                ...
                try:
                    ...
                    logger.warning(
                        f"Appium server on port {self.config.port} did not stop gracefully, killing"
                    )
                    self.process.kill()
...
class AppiumController:
    ...
    async def create_session(
        ...
        fingerprint_script: Optional[str] = None,
    ) -> Optional[str]:
    ...
    async def take_screenshot(
        self, session_id: str, save_path: Optional[str] = None
    ) -> Optional[str]:
    ...
    async def click_element(
        self, session_id: str, locator: Tuple[str, str], timeout: int = 10
    ) -> bool:
