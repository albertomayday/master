...
from ..utils.performance import performance_monitor, register_resource
...
class DeviceFarmYoloDetector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.device = self.model_config.get(
            "device", "cuda" if torch.cuda.is_available() else "cpu"
        )
        self.confidence_threshold = self.model_config.get("confidence_threshold", 0.7)

        # Performance optimization settings
        self.batch_size = self.model_config.get("batch_size", 4)
        self.cache_size = self.model_config.get("cache_size", 100)
        self.enable_memory_optimization = self.model_config.get("memory_optimization", True)
        ...
        register_resource("yolo_detector", self, self._cleanup)
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            if self.enable_memory_optimization:
                if hasattr(model.model, "half") and self.device == "cuda":
                    model.model.half()  # Use half precision on GPU
                    logger.info("✅ Enabled half precision for GPU inference")
    ...
    def _cleanup(self):
        """Cleanup resources"""
        logger.info("🧹 Cleaning up YOLO detector resources...")
    ...
    @performance_monitor("yolo_device_screenshot")
    async def analyze_device_screenshot(
        self, device_serial: str, save_analysis: bool = True
    ) -> Dict[str, Any]:
        """
        Take screenshot from device and analyze with YOLO (optimized version)
        ...
        try:
            ...
            # Check cache first
            cache_key = f"{device_serial}_{os.path.getmtime(screenshot_path)}"
            if cache_key in self._prediction_cache:
                logger.debug(f"📋 Using cached prediction for {device_serial}")
            ...
            # Cache result
            self._cache_prediction(cache_key, analysis_result)
            ...
            logger.info(
                f"Screenshot analysis completed for device {device_serial}: {len(analysis_result['detections'])} detections"
            )
                    ...
                    for i, box in enumerate(boxes):
                        ...
                        if confidence >= self.confidence_threshold:
                            detection = {
                                "id": i,
                                "class_id": class_id,
                                "class_name": self.tiktok_classes.get(
        ...
        for detection in detections:
            ...
            if class_name in ["like_button", "video_player"] and confidence > 0.8:
                recommendations["safe_actions"].append(
                    {"action": "tap", "element": element_info, "risk_level": "low"}
                )

            # Risky actions (high risk)
            elif (
                class_name in ["comment_button", "follow_button", "share_button"]
                and confidence > 0.9
            ):
                recommendations["risky_actions"].append(
                    {"action": "tap", "element": element_info, "risk_level": "high"}
                )

            # All clickable elements
            if (
                class_name
                in self.automation_contexts["engagement"] + self.automation_contexts["navigation"]
            ):
    ...
    async def batch_analyze_devices(self, device_serials: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                batch_results[device_serial] = {"success": False, "error": str(result)}
            else:
                batch_results[device_serial] = {"success": True, "analysis": result}

        return batch_results

    async def detect_anomalies(
        self, device_serial: str, historical_data: Optional[List] = None
    ) -> Dict[str, Any]:
        """Detect anomalies in current screenshot compared to historical patterns"""
        try:
            # Get current analysis
            current_analysis = await self.analyze_device_screenshot(
            ...
            low_conf_ratio = len([d for d in detections if d["confidence"] < 0.6]) / max(
                len(detections), 1
            )
    ...
    async def _save_analysis_results(self, device_serial: str, analysis_result: Dict[str, Any]):
        ...
        try:
            ...
            config = get_config()
            analysis_dir = (
                Path(config.raw_config.get("data_dir", "./data")) / "analysis" / device_serial
            )
    ...
    def _cache_prediction(self, cache_key: str, result: Dict[str, Any]):
        """Cache prediction result with size management"""
    ...
    def clear_cache(self):
        ...
        self._prediction_cache.clear()
        logger.info("🧹 YOLO prediction cache cleared")
    ...
    async def optimize_memory(self):
        ...
        logger.info("🧠 Optimizing YOLO detector memory...")
        ...
        logger.info("✅ YOLO memory optimization completed")
...
async def yolo_screenshot_task_handler(
    device_serial: str, parameters: Dict[str, Any]
) -> Dict[str, Any]:
...
async def yolo_anomaly_detection_task_handler(
    device_serial: str, parameters: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    anomaly_result = await detector.detect_anomalies(
        device_serial=device_serial, historical_data=parameters.get("historical_data")
    )
...
async def yolo_batch_analysis_task_handler(
    device_serial: str, parameters: Dict[str, Any]
) -> Dict[str, Any]:
