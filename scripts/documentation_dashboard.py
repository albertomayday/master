...
class DocumentationDashboard:
    ...
    def render_sidebar(self):
        ...
        selected = st.sidebar.radio(
            "Navigate to:", list(menu_options.keys()), format_func=lambda x: menu_options[x]
        )
    ...
    def render_home_page(self):
        ...
        with col1:
            st.metric("📄 Total Documents", len(docs), delta="+2 this week")

        with col2:
            total_words = sum(doc.word_count for doc in docs)
            st.metric("📝 Total Words", f"{total_words:,}", delta="+1,250 this week")

        with col3:
            total_code_blocks = sum(doc.code_blocks for doc in docs)
            st.metric("💻 Code Examples", total_code_blocks, delta="+15 this week")

        with col4:
            coverage = self.calculate_documentation_coverage()
            st.metric("📊 Coverage", f"{coverage:.1f}%", delta="+5.2% this week")
    ...
    def render_search_page(self):
        ...
        with col2:
            search_type = st.selectbox("Search Type", ["Smart", "Exact", "Fuzzy", "Code Only"])
        ...
        with st.expander("🔧 Advanced Search Options"):
            ...
            with col3:
                date_range = st.date_input(
                    "Modified After", value=datetime.now() - timedelta(days=30)
                )
        ...
        if query:
            ...
            if results:
                ...
                for i, result in enumerate(results[:20]):  # Limit to top 20
                    with st.expander(
                        f"📄 {result.file_name} - {result.section} (Score: {result.relevance_score:.2f})"
                    ):
                        col1, col2 = st.columns([3, 1])
    ...
    def render_browse_page(self):
        ...
        selected_doc_name = st.selectbox(
            ...
            doc_names,
            index=(
                doc_names.index(st.session_state.get("selected_doc", doc_names[0]))
    ...
    def render_analytics_page(self):
        ...
        with col1:
            ...
            if PANDAS_AVAILABLE and PLOTLY_AVAILABLE:
                ...
                fig_views = px.line(
                    df_views, x="Date", y="Views", title="Daily Documentation Views"
                )
        ...
        with col2:
            ...
            if PLOTLY_AVAILABLE:
                fig_search = px.bar(x=search_terms, y=search_counts, title="Top Search Terms")
        ...
        for i, doc_stat in enumerate(popular_docs[:10]):
            ...
            with col5:
                ...
                st.markdown(
                    f"<span style='color: {color}'>{bounce_rate:.0f}% bounce</span>",
                    unsafe_allow_html=True,
                )
    ...
    def render_admin_page(self):
        ...
        # Admin dashboard
        tabs = st.tabs(
            ["📊 Overview", "🔧 Maintenance", "📝 Content Management", "👥 User Management"]
        )
        ...
        with tabs[0]:  # Overview
            ...
            with col1:
                ...
                st.write(
                    f"• Avg file size: {sum(doc.size for doc in docs) / len(docs) / 1024:.1f} KB"
                )
        ...
        with tabs[2]:  # Content Management
            ...
            with col1:
                ...
                update_frequency = st.selectbox(
                    "Update Frequency", ["Every 6 hours", "Daily", "Weekly", "Manual only"]
                )
    ...
    def search_in_document(
        self, doc: DocumentationFile, query: str, search_type: str, section_filter: List[str] = None
    ) -> List[SearchResult]:
        ...
        try:
            ...
            for i, section_content in enumerate(sections):
                ...
                if section_filter and "All" not in section_filter:
                    if not any(
                        filter_name.lower() in section_name.lower()
                        for filter_name in section_filter
                    ):
                        continue
    ...
    def calculate_documentation_coverage(self) -> float:
        ...
        recent_count = sum(1 for doc in docs if (datetime.now() - doc.last_modified).days <= 30)
    ...
    def calculate_health_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            recent_docs = sum(1 for doc in docs if (datetime.now() - doc.last_modified).days <= 30)
